...
def get_db_connection():
    # Saat deploy, aplikasi akan otomatis membaca URL dari konfigurasi Render
    DATABASE_URL = os.environ.get('DATABASE_URL')
    if not DATABASE_URL:
        raise ValueError("DATABASE_URL belum disetel di environment variables!")
...
def init_db():
    ...
    # Auto Migration untuk Kolom Nominal (Abaikan error jika kolom sudah ada)
    try:
...
# Inisialisasi Database hanya jika URL tersedia
...
def hapus_produk(id):
    ...
    flash('Produk berhasil dihapus!', 'success'); return redirect(url_for('produk'))
...
if __name__ == '__main__':
    # gunicorn app:app akan digunakan di Render
